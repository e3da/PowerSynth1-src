...
class CornerStitch_Tmodel_API:
    ...
    def import_layer_stack(self, filename=None):  # Import layer stack from CSV file
        ...
        if filename == None:
            layer_stack_csv_file = QFileDialog.getOpenFileName(self.thermal_main, "Select Layer Stack File", prev_folder, "CSV Files (*.csv)")
        ...
        for k in self.comp_dict:
            ...
            if isinstance(comp, Part):
                if comp.type == 1: # if this is a component
                    ...
                    value = eg.enterbox(msg='Enter a power flow for this device:'+comp.layout_component_id)
                    self.dev_powerload_table[comp.layout_component_id] = float(value)
        print "getting convection coeff"
        value = eg.enterbox(msg='Enter baseplate convection coefficient')
        self.bp_conv = float(value)
        value = eg.enterbox(msg='Enter ambient temp')
    ...
    def set_up_thermal_props(self,layout_data=None): # for analytical model of a simple 6 layers
        ...
        t_bp = Baseplate(width=baseplate.dimensions[0] * 1e-3,
    ...
    def dev_result_table_eval(self,layout_data):
