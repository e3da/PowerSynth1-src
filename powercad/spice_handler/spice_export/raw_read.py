        ...
        while line != '':
            ...
            if keyword == 'title':
                ...
                if not self.real:
                    ...
                    alldata = np.fromfile(simfile, count=2 * self.novariables * self.nopoints, dtype='float64')
                    if np.size(alldata) > 1:
                        for pno in range(self.nopoints):
                            for vno in range(self.novariables):
                                index = self.novariables * pno * 2 + vno * 2
                                if index != np.size(alldata):
                                    self.values[vno][pno] = alldata[index]
                    else:
                        for pno in range(self.nopoints):
                            self.values[0][pno] = alldata
...
if __name__ == '__main__':
    data = SimData("C:\Users\qmle\Desktop\Documents\Conferences\Device_optimization//device file//CV_GaN.raw")
    ...
    print dictionary
    plt.figure(1)
    plt.plot(dictionary['v2'], dictionary['V(m1:nciss)'])  # put the name of the net you want here
    plt.plot(dictionary['v2'], dictionary['V(m1:ncoss)'])  # put the name of the net you want here
    plt.plot(dictionary['v2'], dictionary['V(m1:ncrss)'])  # put the name of the net you want here
