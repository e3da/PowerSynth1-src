...
class RL_circuit():
    ...
    def _add_termial(self, node, ground=0):
        #newport = self.node_dict[node]
        Equiv_name = 'Bt_' + str(node)
        self._graph_add_comp(Equiv_name, node, ground, 1e-6)
    ...
    def build_current_info(self):
        ...
        for i in range(len(self.element)):
            ...
            if (x == 'B'): # add this for current through source or (x == 'V'):
        ...
        for i in range(len(self.element)):
            ...
            if (x == 'C'):
                ...
                if n2 == 0:
                    self.G[n2 - 1, n2 - 1] += g
            # B  C MATRIX
            ''' Uncomment to compute current through vsource'''
            '''
            if x == 'V':
    ...
    def solve_iv(self):
        ...
        if debug:
            print "forming matrix_formation time", time.time() - _form_time
            _form_time = time.time()
