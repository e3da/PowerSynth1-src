...
class New_layout_engine():
    ...
    def create_cornerstitch(self, input_rects=None, size=None, islands=None):
        ...
        input = self.cornerstitch.read_input('list',
                                             Rect_list=input_rects)  # Makes the rectangles compaitble to new layout engine input format
        self.Htree, self.Vtree = self.cornerstitch.input_processing(input, size[0], size[
            1])  # creates horizontal and vertical corner stitch layouts
        patches, combined_graph = self.cornerstitch.draw_layout(rects=input_rects, Htree=self.Htree,
                                                                Vtree=self.Vtree)  # collects initial layout patches and combined HCS,VCS points as a graph for mode-3 representation
    ...
    def mode_zero(self):  # evaluates mode 0(minimum sized layouts)
        ...
        Evaluated_X, Evaluated_Y = CG1.evaluation(Htree=self.Htree, Vtree=self.Vtree, N=None, W=None, H=None, XLoc=None,
                                                  YLoc=None, seed=None, individual=None, Types=self.Types)  #
            ...
            if len(island.child) == 0:
                ...
                for element in island.elements:
                    ...
                    for rect in Vtree.vNodeList[0].stitchList:
                        ...
                        if rect.cell.x == element[1] and rect.cell.y == element[2] and rect.cell.type == str(
                                element[0]):
    ...
    def form_cs_island(self, islands=None, Htree=None, Vtree=None):
        ...
        for node in Htree.hNodeList:
            if node.child == []:
                ...
                HorizontalNodeList.append(
                    node)  # only appending all horizontal tree nodes which have children. Nodes having no children are not included

        for node in Vtree.vNodeList:
            if node.child == []:
                continue
            else:
                VerticalNodeList.append(
                    node)  # only appending all vertical tree nodes which have children. Nodes having no children are not included
                ...
                for node in HorizontalNodeList:
                    for i in node.stitchList:
                        if rect[1] == i.cell.x and rect[2] == i.cell.y and rect[3] == i.getWidth() and rect[
                            ...
                            r = [rect[0], rect[1], rect[2], rect[3], rect[4], rect[5], rect[8],
                                 i.nodeId]  # type,x,y,width,height,name, hierarchy_level, nodeId
                    ...
                    for node in HorizontalNodeList:
                        for i in node.stitchList:
                            if rect[1] == i.cell.x and rect[2] == i.cell.y and rect[3] == i.getWidth() and rect[
                                4] == i.getHeight() and rect[0] == i.cell.type:
                                r = [rect[0], rect[1], rect[2], rect[3], rect[4], rect[5], rect[8],
                                     node.id]  # type,x,y,width,height,name, hierarchy_level, parent nodeId
                                cs_child.append(r)
                                cs_mapped_input[rect[5]] = [i, node, rect[8]]
    ...
    if level == 0:
        ...
        for k, v in Total_H.items():
            ...
            for i in range(len(v)):
                ...
                type = ['EMPTY', 'Type_1', 'Type_2', 'Type_3', 'Type_4', 'Type_5', 'Type_6', 'Type_7', 'Type_8',
                        'Type_9']
                for i in Rectangles:
                    for t in type:
                        if i[4] == t:
