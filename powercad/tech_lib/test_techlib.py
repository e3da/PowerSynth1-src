...
def get_device():
    ...
    bl_g = BondwireLanding((4.4, 1.2), BondwireLanding.SIGNAL)
    bl_s1 = BondwireLanding((1.2, 2.0), BondwireLanding.POWER)
    bl_s2 = BondwireLanding((2.4, 1.2), BondwireLanding.POWER)
    bl_s3 = BondwireLanding((1.2, 0.4), BondwireLanding.POWER)
    device_model = open_device_model('powerMOSVA.va')
    dev = Device('CPMF-1200-S080B SiC-MOS', (4.8, 2.4, 0.35), device_props, Device.TRANSISTOR,
                 [bl_g, bl_s1, bl_s2, bl_s3], device_model, 1)
    return dev
...
def get_substrate2():
    ...
    substrate = Substrate('Cu-AlN-Cu 12-25-12 mils', isolation_props, metal_props, 0.3, 0.64)
    return substrate
...
def get_baseplate():
    # properties
    base_props = MaterialProperties('MarkeTech Mo70Cu Moly', 190.0, 385.0, 9700.0, 1.70e-8, None, 1.0)
...
def get_power_lead():
    ...
    lead = Lead('Aluminium Busbar Lead 18x5x0.5x10mm', Lead.POWER, Lead.BUSBAR, (18, 5.0, 0.5, 10.0), ld_props)
    return lead
