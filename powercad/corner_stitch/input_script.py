...
class ScriptInputMethod():
    ...
    def update_constraint_table(self):
        ...
        Types_init=self.component_to_cs_type.values() # Get all initial types name
        Types = ["EMPTY"]
        print Types_init
        type_name_to_id = {} # To sort it in the same order first get the name to index relationship
        for i in Types_init:
            if i!= 'EMPTY':
                t=i.strip('Type_')
                type_name_to_id[i]=t # get the index to sort later
        sorted_d = OrderedDict(sorted(type_name_to_id.items(), key=lambda x: x[1])) # sort and keep the items in
                                                                                    # ordered dictionary
        Types= Types+sorted_d.keys() # Finally !!
